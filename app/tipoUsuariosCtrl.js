app.controller('tipoUsuariosCtrl', function ($scope, $modal, $filter, Data) {
    $scope.usuario = {};
    Data.get('tusuarios').then(function(data){
        $scope.usuarios = data.data;
    });

    $scope.deleteUsuario = function(usuario){
        if(confirm("Estas seguro de eliminar al usuario?")){
            Data.delete("tusuarios/"+usuario.id).then(function(result){
...
app.controller('tipoUsuarioEditCtrl', function ($scope, $modalInstance, item, Data) {
  ...
  $scope.usuario = angular.copy(item);
        ...
        $scope.title = (item.id > 0) ? 'Editar tipo de usuario' : 'Agregar tipo de usuario';
        $scope.buttonText = (item.id > 0) ? 'Actualizar tipo de usuario' : 'Agregar nuevo tipo de usuario';
