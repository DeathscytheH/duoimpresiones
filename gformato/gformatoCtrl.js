app.controller('gformatoCtrl', function ($scope, $modal, $filter, Data) {
    $scope.usuario = {};
    ...
    $scope.deleteUsuario = function(usuario){
        if(confirm("Estas seguro de eliminar el producto?")){
            Data.delete("gformato/"+usuario.id).then(function(result){
    ...
    $scope.open = function (p,size) {
        var modalInstance = $modal.open({
          templateUrl: 'gformato/gformatoEdit.html',
          controller: 'gformatoEditCtrl',
          size: size,
          resolve: {
            item: function () {
              return p;
...
app.controller('gformatoEditCtrl', function ($scope, $modalInstance, item, Data) {
  ...
  $scope.usuario = angular.copy(item);
        ...
        $scope.saveUsuario = function (usuario) {
            if(usuario.id > 0){
                Data.put('gformato/'+usuario.id, usuario).then(function (result) {
                ...
                Data.post('gformato', usuario).then(function (result) {
                    if(result.status != 'error'){
                        var x = angular.copy(usuario);
                        x.save = 'insert';
                        x.id = result.data;
