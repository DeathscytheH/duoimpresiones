app.controller('tiemposAcabadosCtrl', function ($scope, $modal, $filter, Data) {
    $scope.usuario = {};
    Data.get('tiemposAcabados').then(function(data){
        $scope.usuarios = data.data;
    });

    $scope.deleteUsuario = function(usuario){
        if(confirm("Estas seguro de eliminar la variable?")){
            Data.delete("tiemposAcabados/"+usuario.id).then(function(result){
    ...
    $scope.open = function (p,size) {
        var modalInstance = $modal.open({
          templateUrl: 'tiempos/acabados/tiemposAcabadosEdit.html',
          controller: 'tiemposAcabadosEditCtrl',
          size: size,
          resolve: {
            item: function () {
              return p;
...
app.controller('tiemposAcabadosEditCtrl', function ($scope, $modalInstance, item, Data) {
  ...
  $scope.usuario = angular.copy(item);
        ...
        $scope.saveUsuario = function (usuario) {
            if(usuario.id > 0){
                Data.put('tiemposAcabados/'+usuario.id, usuario).then(function (result) {
                ...
                Data.post('tiemposAcabados', usuario).then(function (result) {
                    if(result.status != 'error'){
                        var x = angular.copy(usuario);
                        x.save = 'insert';
                        x.id = result.data;
